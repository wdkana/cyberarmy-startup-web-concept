<div class="panel panel-flat border-left-lg border-left-danger">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#no_sim').blur(function () {
                var no_sim = $(this).val();
                var len = no_sim.length;
                if (len > 0 && len < 12) {
                    $(this).parent().find('.text-warning').text("");
                    $(this).parent().find('.text-warning').text("Minimal 12 karakter");
                    return apply_feedback_error(this);
                } else {
                    if (len > 0) { //jika ada isinya
                        if (!valid_no_sim(no_sim)) { //jika username tidak valid
                            $(this).parent().find('.text-warning').text("");
                            $(this).parent().find('.text-warning').text("No Sim Tidak Valid");
                            return apply_feedback_error(this);
                        } else {
                            if (len > 12) { //jika karakter >30
                                $(this).parent().find('.text-warning').text("");
                                ...
                                var valid = false;
                                $.ajax({
                                    url: "<?php echo site_url('verifikasi/ceksim');?>",
                                    type: "POST",
                                    data: "no_sim=" + no_sim,
                                    dataType: "text",
                                    success: function (data) {
                                        if (data == 0) { //pada file check email.php, apabila email sudah ada di database makan akan mengembalikan nilai 0
                                            $('#no_sim').parent().find('.text-warning').text("");
                                            $('#no_sim').parent().find('.text-warning').text("No SIM sudah ada");
                                            return apply_feedback_error('#no_sim');
                                        }
                                    }
                                });
                            }
            ...
            $('#formInput').submit(function (e) {
                ...
                if (valid) {
                    swal({
                        ...
                        text: "Data SIM akan di Simpan Ke Database, Apakah data Sudah Benar?",
                        type: "info",
                        ...
                        $.ajax({
                            url: "<?php echo site_url('verifikasi/prosess_sim');?>",
                            type: "POST",
        ...
        function valid_no_sim(no_sim) {
            var pola = new RegExp(/^[0-9-+]+$/);
            return pola.test(no_sim);
        }
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="content">
                                    ...
                                    <?php else  :?>
                                    <?php echo form_open_multipart('verifikasi/prosess_sim',['role'=>'form','id'=>'formInput']);?>
                                    <div class="row">
                                        <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group form-group-lg has-feedback">
                                                <label>No SIM</label>
                                                <input type="number" class="form-control nik textbox" required="required" id="no_sim" name="no_sim" placeholder="No SIM">
                                    ...
                                    <span class="text-danger">
                                        <i class="icon-info22"></i>&nbsp;Peringatan: Di Usahakan Data SIM Sesuai dengan SIM Asli , agar akun terverifikasi
                                    </span>
                                    <button type="submit" class="btn btn-info btn-fill pull-right">Verifikasi SIM</button>
