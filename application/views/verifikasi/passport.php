    ...
    <script>
        $(document).ready(function () {
            ...
            $('#passport').blur(function () {
                ...
                if (len > 0 && len < 9) {
                    $(this).parent().find('.text-warning').text("");
                    $(this).parent().find('.text-warning').text("Maximal 9 karakter");
                    return apply_feedback_error(this);
                } else {
                    if (len > 0) { //jika ada isinya
                        if (!valid_passport(passport)) { //jika username tidak valid
                            $(this).parent().find('.text-warning').text("");
                            ...
                            if (len > 16) { //jika karakter >30
                                ...
                                var valid = false;
                                $.ajax({
                                    url: "<?php echo site_url('verifikasi/cekpassport');?>",
                                    type: "POST",
                                    data: "passport=" + passport,
                                    dataType: "text",
                                    success: function (data) {
                                        if (data == 0) { //pada file check email.php, apabila email sudah ada di database makan akan mengembalikan nilai 0
                                            $('#passport').parent().find('.text-warning').text("");
                                            $('#passport').parent().find('.text-warning').text("No SIM sudah ada");
                                            return apply_feedback_error('#passport');
                                        }
                                    }
                                });
                            }
            ...
            $('#formInput').submit(function (e) {
                ...
                if (valid) {
                    swal({
                        ...
                        $.ajax({
                            url: "<?php echo site_url('verifikasi/prosess_passport');?>",
                            type: "POST",
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="content">
                                    ...
                                    <?php else  :?>
                                    <?php echo form_open_multipart('verifikasi/prosess_passport',['id'=>'formInput']);?>
                                    <div class="row">
                                        <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group form-group-lg has-feedback">
                                                ...
                                                <input type="number" class="form-control passport textbox" required="required" id="passport" name="passport" placeholder="Passport">
                                    ...
                                    <span class="text-danger">
                                        <i class="icon-info22"></i>&nbsp;Peringatan: Di Usahakan Data Passport Sesuai dengan Passport Asli , agar akun terverifikasi
                                        </span>
                                    <button type="submit" class="btn btn-info btn-fill pull-right">Verifikasi Passport</button>
